...
export function AuthService($location, $http, $cookies, $q, socket, appConfig, Util, User) {
  'ngInject';
  ...
  var Auth = {
    ...
    subscribe() {
      if (!subscribed) {
        socket.subscribe(currentUser._id, (err) => {
          if (err) {
            return;
          }
    ...
    unsubscribe() {
      socket.unsubscribe(currentUser._id, (err) => {
        if (!err) {
          subscribed = false;
